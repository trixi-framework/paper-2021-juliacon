...
computing relying on floating point operations is that Julia does not enable
fused multiply-add (FMA) contraction by default, while GCC and Intel compilers
do\footnote{An interesting coincidence is that not only Julia but most LLVM-based
frontends tend not to enable FMA contraction by default.}.
Thus, one must opt-in to these optimizations explicitly.
...
which makes it convenient to use Julia's function \lstinline{muladd}\footnote{\url{https://docs.julialang.org/en/v1/base/math/\#Base.muladd}}.
