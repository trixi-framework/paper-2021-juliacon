...
in CFD over general multi-physics coupling strategies to the required high-order
numerical methods themselves. In addition, we are academics and involved in teaching
students in these areas of science. Thus, we would like to have a code that is
\begin{enumerate}
  ...
  \item fast enough to be useful not only for small toy problems
\end{enumerate}
Additionally, we are greedy\footnote{See \url{https://julialang.org/blog/2012/02/why-we-created-julia/}} and want to have all these features in a single code.
Roughly one year of collaborative work has resulted in the current version of
\trixi\footnote{https://github.com/trixi-framework/Trixi.jl}, which provides adaptive
...
\todo{TODO: Jesse wants to write about the \trixi community} %TODO: Jesse
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Forests of qudtrees/octrees with \texttt{p4est} \cite{burstedde2011p4est}
...
or closed source) codes designed as monolithic applications that can
simulate a specific setup. Such monolithic codes can often be configured in limited amounts by
...
a simulation setup for \trixi are called elixirs. As of version v0.3.50, \trixi
...
more discontinuous Galerkin methods on simplex elements via StartUpDG.jl\footnote{\url{https://github.com/jlchan/StartUpDG.jl}} in \trixi.
This is feasible because the modular design of \trixi gives developers the flexibility to use
...
\todo{TODO: Be careful how to formulate this, be neutral and avoid making people angry} %TODO
...
In addition, there are some Julia packages dedicated to solve specific time-dependent
...
can be used with \trixi by changing a single line of code in an elixir file. In contrast, researchers
working without external dependencies would need to use a day or two to digest
all the details and implement such time integration methods in their own codes, making it much
...
two-language problem --- at least to a large extent. Highly efficient low-level
...
users to contribute to a package and act as developers. This also contributes
...
under the permissive MIT license, including \trixi. This allows using Julia
without the need to pay for commercial software. In particular, students can
...
However, we do not expect unsolvable problems there, as Julia is not generically
slower than traditional compiled HPC programming languages. Nevertheless, it
appears to be more complicated to scale Julia to HPC while still keeping the
...
time. Nevertheless, it pays off in our experience and we do not want to miss the
...
\begin{itemize}
  ...
  \item Shall we compare \trixi (in terms of structure, not performance!)
        to other open source frameworks such as MFEM,
        NGSolve, DealII, DUNE, FENICS, Firedrake, ClawPack, ...? Definitely not
        explicitly to not make people angry, but maybe implicitly without
        mentioning their names?
        \begin{itemize}
          \item Gregor will think about it!
        \end{itemize}
  \item Are there other aspects we should discuss?
  \item Check affiliations
  \item Check order of authors
  \item Maybe include some cool pictures? Give a flavor of the kind of simulations
        you can do with \trixi? Pictures trigger people! For example after the
        list of main features?
  \item Summary of community, collaborators, and users: GitHub statistics, contributors,
        Vikram and Chandrashekar, Steven Frankel and Philipp Öffner (maybe), ...
        Related to the relatively large number of students!
        $\to$ Jesse will make a PR!
  \item A simple example where we really use multiple dispatch?
        \begin{itemize}
          \item Just pass functions around
          \item Dispatch on parameters of DG struct
        \end{itemize}
\end{itemize}
\begin{quote}
