...
in hyperbolic partial differential equations (PDEs), computational fluid dynamics (CFD),
...
\trixi\footnote{https://github.com/trixi-framework/Trixi.jl}, providing adaptive
...
example above. Moreover, there is no hidden global state and multiple instances
...
the \lstinline{solver} but are still very efficient. In particular, this allows
to reuse the same numerical fluxes for discontinuous Galerkin methods, finite
...
simple variants on a subset of the available meshes, selected via multiple dispatch.
...
a lot of tools for using corresponding CI setups, preparing and publishing
...
back-ends lead to a well-known barrier between user and developers of a library.
...
two-language problem --- at least to a large extend. Highly efficient low-level
...
comparisons with established Fortran codes. Having worked on \trixi for a bit
...
However, we do not expect unsolvable problems there since Julia is not generically
