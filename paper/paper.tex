...
Figure~\ref{fig:colliding_flows} demonstrates entropy-dissipative shock-capturing
methods with positivity-preserving limiters and adaptive mesh refinement applied
to supersonic colliding flows inspired by astrophysical setups.

\begin{figure}[htbp]
  \begin{subfigure}{0.53\linewidth}
    \includegraphics[width=\textwidth]{../figures/colliding_flows_density_t15}
    \caption{Density at time $t = 15$.}
  \end{subfigure}%
  \hspace*{\fill}
  \begin{subfigure}{0.47\linewidth}
    \includegraphics[width=\textwidth]{../figures/colliding_flows_mesh_t15}
    \caption{Mesh at time $t = 15$.}
  \end{subfigure}%
  \\
  \begin{subfigure}{0.53\linewidth}
    \includegraphics[width=\textwidth]{../figures/colliding_flows_density_t25}
    \caption{Density at time $t = 25$.}
  \end{subfigure}%
  \hspace*{\fill}
  \begin{subfigure}{0.47\linewidth}
    \includegraphics[width=\textwidth]{../figures/colliding_flows_mesh_t25}
    \caption{Mesh at time $t = 25$.}
  \end{subfigure}%
  \caption{Numerical solutions of two colliding supersonic flows using
           entropy-dissipative shock-capturing methods, positivity-preserving
           limiters, and adaptive mesh refinement for the compressible Euler
           equations.}
  \label{fig:colliding_flows}
\end{figure}
...
the \lstinline{solver} while retaining efficiency. In particular, this allows
...
discontinuous Galerkin methods on simplices via StartUpDG.jl\footnote{\url{https://github.com/jlchan/StartUpDG.jl}} 
in \trixi. This becomes feasible since not every feature combination needs to be considered, 
enabling developers to introduce variants of existing solvers on only a subset of the available
mesh types via multiple dispatch.
