...
v0.3.57 (August 2021), \trixi comes with approximately 200 example elixirs. A basic elixir
looks as follows:
...
simplicity over abstractions in the code, it remains comparatively straightforward to modify
...
Further, we expect to be able to achieve similar performance with either language, and that
remaining performance differences are due to how much time and effort has been spent to optimize
each code.  \trixi owes its performance optimizations in part to the code introspection and
...
general, the performance of MPI programs written in Julia is to be comparable to C programs
...
Nevertheless, it has paid off in our experience and we do not want to miss the
many useful features of \trixi enabled by Julia and its ecosystem. Thus, while there is
