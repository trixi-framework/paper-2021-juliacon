...
flow instability problem. Figure~\ref{fig:pressure_waves} shows the approximation
...
the 3D ideal MHD equations~\cite{altmann2012}. These initial conditions, available in FLUXO and
...
For the flux differencing solver, that introduces an additional numerical volume flux function, we use an entropy-conservative flux
function in both the volume and at the surface. For the compressible Euler equations this flux is also kinetic energy preserving~\cite{ranocha2020icosahom}
...
in the volume of the flux differencing DG solver, albeit with additional computational cost.

All simulations for the performance comparison were run on the High Performance Computing (HPC) system Tetralith
...
The serial performance results we run on a single core.
...
To assess the performance of the two codes we vary the polynomial degree (polydeg) in the range $[3,15]$.
...
\begin{equation}
\label{eqn:pid}
\text{PID} = \frac{\text{wall-clock-time}}{\#\text{time steps}\cdot\#\text{elements}\cdot(\text{polydeg}+1)^3}.
\end{equation}
In other words, the PID acts as a metric for the run time required for the right hand side (RHS) evaluation of each DOF.
    ...
    \begin{tikzpicture}[
      ...
      \begin{axis}[
        ...
        \addplot[blue, mark=*, solid] % FLUXO, weak form
          table [x index=0, y index=2]{../new_pid_data/pids_euler_weak.dat};
...
%           \addlegendentry{\texttt{TreeMesh} (\trixi)}
        \addplot[red, mark=diamond*, solid] % StructuredMesh, weak form
          table [x index=0, y index=1]{../new_pid_data/pids_euler_weak.dat};
        ...
        \addplot[blue, mark=*, dashed] % FLUXO, flux differencing
          table [x index=0, y index=2]{../new_pid_data/pids_euler_ranocha.dat};
...
%           table [x index=0, y index=1]{../data/pids_euler_ranocha.dat};
        \addplot[red, mark=diamond*, dashed] % StructuredMesh, flux differencing
          table [x index=0, y index=1]{../new_pid_data/pids_euler_ranocha.dat};
    ...
    \begin{tikzpicture}[
      ...
      \begin{axis}[
        ...
        \addplot[blue, mark=*, solid, forget plot] % FLUXO, weak form
          table [x index=0, y index=2]{../new_pid_data/pids_euler_weak_relative.dat};
...
%           table [x index=0, y index=1]{../data/pids_euler_weak_relative.dat};
        \addplot[red, mark=diamond*, solid] % StructuredMesh, weak form
          table [x index=0, y index=1]{../new_pid_data/pids_euler_weak_relative.dat};
          \addlegendentry{Weak form}

        \addplot[blue, mark=*, dashed, forget plot] % FLUXO, flux differencing
          table [x index=0, y index=2]{../new_pid_data/pids_euler_ranocha_relative.dat};
...
%           table [x index=0, y index=1]{../data/pids_euler_ranocha_relative.dat};
        \addplot[red, mark=diamond*, dashed] % StructuredMesh, flux differencing
          table [x index=0, y index=1]{../new_pid_data/pids_euler_ranocha_relative.dat};
  ...
  \caption{Run time per right-hand side evaluation and degree of freedom for
           ...
           in FLUXO (Fortran) and \trixi (Julia).}
  \label{fig:PID-Euler}
\end{figure}
...
\centering
  \begin{subfigure}{\linewidth}
    \begin{tikzpicture}[
      ...
      \begin{axis}[
        ...
        \addplot[blue, mark=*, solid] % FLUXO, weak form
          table [x index=0, y index=2]{../new_pid_data/pids_mhd_weak.dat};
          \addlegendentry{FLUXO}
        \addplot[red, mark=diamond*, solid] % StructuredMesh, weak form
          table [x index=0, y index=1]{../new_pid_data/pids_mhd_weak.dat};
          \addlegendentry{\trixi}
        \node at (axis cs:13,3.5e-7) {Weak form};

        \addplot[blue, mark=*, dashed] % FLUXO, flux differencing
          table [x index=0, y index=2]{../new_pid_data/pids_mhd_flogor.dat};
        \addplot[red, mark=diamond*, dashed] % StructuredMesh, flux differencing
          table [x index=0, y index=1]{../new_pid_data/pids_mhd_flogor.dat};
        \node [rotate=15] at (axis cs:9,8.2e-7) {Flux differencing};
        \draw (axis cs:9,9.2e-7) -- (axis cs:8.5,10.2e-7);
        \draw (axis cs:9,7.2e-7) -- (axis cs:8.5,5.2e-7);
      \end{axis}
  ...
  \begin{subfigure}{\linewidth}
    \begin{tikzpicture}[
      ...
      \begin{axis}[
          ...
          ymin=0.0,
          ymax=1.1,
          ytick={0, 0.2, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1},
          grid=major,
        ]
        \addplot[blue, mark=*, solid, forget plot] % FLUXO, weak form
          table [x index=0, y index=2]{../new_pid_data/pids_mhd_weak_relative.dat};
        \addplot[red, mark=diamond*, solid] % StructuredMesh, weak form
          table [x index=0, y index=1]{../new_pid_data/pids_mhd_weak_relative.dat};
          \addlegendentry{Weak form}

        \addplot[blue, mark=*, dashed, forget plot] % FLUXO, flux differencing
          table [x index=0, y index=2]{../new_pid_data/pids_mhd_flogor_relative.dat};
        \addplot[red, mark=diamond*, dashed] % StructuredMesh, flux differencing
          table [x index=0, y index=1]{../new_pid_data/pids_mhd_flogor_relative.dat};
          \addlegendentry{Flux differencing}
      \end{axis}
    \end{tikzpicture}
    \caption{Run time relative to FLUXO.}
  \end{subfigure}%
  \caption{Run time per right-hand side evaluation and degree of freedom for
           different DG discretizations of the 3D ideal MHD equations.}
  \label{fig:PID-MHD}
\end{figure}
...
\todo{TODO: add Intel discussion} % TODO: Andrew

\todo{TODO: alter slightly with MHD results} % TODO: Andrew

\todo{TODO: \trixi runs need redone due to further improvements} % TODO: Andrew
...
on spatial semidiscretizations can benefit immediately by changing only one line
...
\todo{TODO: Gregor, add your funding sources that need to be here} %TODO Gregor
